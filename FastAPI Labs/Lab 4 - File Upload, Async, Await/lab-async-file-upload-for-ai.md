# Lab 4 — Async/Await and File Upload for a Document Q&A Pipeline
...
- **Real API calls**: This lab uses OpenRouter's unified API (OpenAI-compatible). Embedding and chat calls are routed to free-tier models. You will need an OpenRouter API key (see Section 9).
...
3. **Embedding**: Each chunk is converted into a vector (embedding) using the `openai/text-embedding-3-small` model served through OpenRouter's `/embeddings` endpoint. This is the step where sequential and concurrent diverge — one calls the API 8 times in a row, the other launches all 8 at once.
...
6. **Answer Generation**: The retrieved chunks are passed as context to the `openrouter/free` chat model, which generates an answer based on the document.
...
```
Sequential: 8 chunks in 6.16s
Concurrent: 8 chunks in 1.63s

Concurrent was 3.8x faster for 8 chunks

Question: What advantage does asyncio.gather provide for document processing?

Retrieved sources:

Answer: Based on the context, the advantage of `asyncio.gather` is that it allows multiple requests (such as eight requests) to launch simultaneously, causing the total wall time to approach that of a single request. This makes it highly efficient for IO-bound document processing tasks without blocking the server.
```

Actual timings depend on network latency and OpenRouter API response times. The exact numbers in the Mermaid diagram (Section 7) are illustrative — your real output may differ.
...
- `openai==3.5.0` — OpenAI-compatible client; pointed at OpenRouter's base URL for both embeddings and chat
- `python-dotenv` — Load API keys from `.env` file
- `python-multipart` — Required by FastAPI for `UploadFile` form data parsing
- `numpy` — Vector math for cosine similarity
- **Embedding model**: `openai/text-embedding-3-small` (via OpenRouter free tier, ~$0 cost)
- **Chat model**: `openrouter/free` (OpenRouter free tier, ~$0 cost)
...
The illustrative numbers above (~4.0s vs ~1.0s) depend on network latency and OpenRouter API response times — your real timings will differ.
...
- The key must be set in a `.env` file as `OPEN_ROUTER_KEY`, or you will be prompted to enter it at runtime (see Section 9)
...
pip install fastapi==0.112.2 pydantic==2.8.2 httpx==0.28.1 openai==3.5.0 python-dotenv==1.2.3 python-multipart numpy
```
...
Get a free key at https://openrouter.ai/keys. If you are using version control, add `.env` to your `.gitignore` to avoid committing your key.

If no `OPEN_ROUTER_KEY` is found in the environment or `.env` file, the notebook will prompt you to enter it at runtime.
...
```python
!pip install fastapi pydantic httpx openai==3.5.0 python-dotenv python-multipart numpy
```

### Cell 2: Imports and Client Setup

We import the modules we need, load the API key from `.env` (or prompt for it), and initialize the `AsyncOpenAI` client pointed at OpenRouter's base URL. NumPy is used for cosine similarity calculations.
...
This is the core API call. We use the **async** client to embed a single text chunk through OpenRouter's OpenAI-compatible `/embeddings` endpoint. This is what makes real concurrency possible — the `await` yields control back to the event loop while the network call is in flight.
